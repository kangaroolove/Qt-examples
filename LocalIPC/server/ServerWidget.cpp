...
ServerWidget::ServerWidget(QWidget *parent)
    : QWidget(parent), m_receiveTextEdit(new QTextEdit(this)),
      m_sendTextEdit(new QTextEdit(this)),
      m_sendButton(new QPushButton("Send", this)),
      m_statusLabel(new QLabel(this)), m_server(new Server(this)),
...
void ServerWidget::onAutoSendButtonClicked() {
  ...
  if (interval.isEmpty()) {
    QMessageBox::critical(this, "Critical",
                          "You didn't enter the timer interval!");
    return;
  }
  ...
  connect(m_timer.get(), &QTimer::timeout, this,
          &ServerWidget::onSendButtonClicked);
  m_timer->start();
}
...
void ServerWidget::bindConnections() {
  connect(m_sendButton, &QPushButton::clicked, this,
          &ServerWidget::onSendButtonClicked);
  connect(m_server, &Server::receiveMessage, this,
          [this](const QString &msg) { m_receiveTextEdit->append(msg); });
  connect(m_autoSendButton, &QPushButton::clicked, this,
          &ServerWidget::onAutoSendButtonClicked);
  connect(m_stopSendButton, &QPushButton::clicked, this,
          [this] { m_timer->stop(); });
}
