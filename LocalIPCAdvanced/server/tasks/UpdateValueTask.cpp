...
void UpdateValueTask::run()
{
    ...
    qInfo()<<"Parameter = "<<m_updateValueInfo.parameter;
    ...
    else if (m_updateValueInfo.parameter == DaqcParameter::INIT)
    {
        ...
        m_daqc->Palette(values.first().toInt(), values.at(1).toInt(), values.last().toInt());
        ...
        m_daqc->Mode2B();
    else if (m_updateValueInfo.parameter == DaqcParameter::D_BASELINE)
        m_daqc->SetDBaseLine(values.first().toInt());
}
