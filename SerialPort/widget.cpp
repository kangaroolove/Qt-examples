...
Widget::Widget(QWidget *parent)
    : QWidget(parent), m_serialPortLabel(new QLabel("Serial Port:")),
      ...
      m_sendLabel(new QLabel("Send:")), m_sendTextEdit(new QTextEdit()),
...
void Widget::onSendClearButtonClicked() { m_sendTextEdit->clear(); }

void Widget::onResponseClearButtonClicked() { m_responseTextEdit->clear(); }
...
void Widget::initBaudRates() {
  m_baudRates = {QSerialPort::Baud1200,  QSerialPort::Baud2400,
  ...
  for (auto it = m_baudRates.begin(); it != m_baudRates.end(); it++)
    m_baudRateComboBox->insertItem(std::distance(m_baudRates.begin(), it),
                                   QString::number(*it));
}
...
void Widget::initDataBits() {
  ...
  for (auto it = m_dataBits.begin(); it != m_dataBits.end(); it++)
    m_dataBitsComboBox->insertItem(std::distance(m_dataBits.begin(), it),
                                   QString::number(*it));
...
void Widget::initFlowControl() {
  m_flowControl = {{QSerialPort::NoFlowControl, tr("No Flow Control")},
  ...
  for (auto it = m_flowControl.begin(); it != m_flowControl.end(); it++)
    m_flowControlComboBox->insertItem(std::distance(m_flowControl.begin(), it),
                                      it->second, it->first);
}

void Widget::initParities() {
  m_parities = {{QSerialPort::NoParity, tr("No Parity")},
  ...
  for (auto it = m_parities.begin(); it != m_parities.end(); it++)
    m_parityComboBox->insertItem(std::distance(m_parities.begin(), it),
                                 it->second, it->first);
}
...
void Widget::initStopBits() {
  ...
  for (auto it = m_stopBits.begin(); it != m_stopBits.end(); it++)
    m_stopBitsComboBox->insertItem(std::distance(m_stopBits.begin(), it),
                                   it->second, it->first);
}
...
void Widget::initPinoutSignals() {
  m_pinoutSignal = {
      ...
      {QSerialPort::SecondaryTransmittedDataSignal,
       tr("Secondary Transmitted Data Signal")},
      {QSerialPort::SecondaryReceivedDataSignal,
       tr("Secondary Received Data Signal")}};

  for (auto it = m_pinoutSignal.begin(); it != m_pinoutSignal.end(); it++)
    m_pinoutSignalComboBox->insertItem(
        std::distance(m_pinoutSignal.begin(), it), it->second, it->first);
}

void Widget::initConnections() {
  connect(m_connectButton, &QPushButton::clicked, this,
          &Widget::onConnectButtonClicked);
  connect(m_sendClearButton, &QPushButton::clicked, this,
          &Widget::onSendClearButtonClicked);
  connect(m_responseClearButton, &QPushButton::clicked, this,
          &Widget::onResponseClearButtonClicked);
  connect(m_sendByHexButton, &QPushButton::clicked, this,
          &Widget::onSendByHexButtonClicked);
  connect(m_sendByAsciiButton, &QPushButton::clicked, this,
...
SerialPortInfo Widget::getSerialPortInfo() {
  ...
  info.flowControl =
      (QSerialPort::FlowControl)m_flowControlComboBox->currentData().toInt();
  info.parity = (QSerialPort::Parity)m_parityComboBox->currentData().toInt();
  info.stopBits =
      (QSerialPort::StopBits)m_stopBitsComboBox->currentData().toInt();
